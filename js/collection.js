(function () {
  ...
  function renderProduct(p) {
    ...
    return (
      '<div class="max-w-sm w-full relative bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-xl transition-shadow duration-300 group">' +
        '<a href="product.html?id=' + encodeURIComponent(p.id) + '" class="block">' +
          ...
          '</div>' +
        '</a>' +
        '<button type="button" class="absolute bottom-4 right-4 bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center translate-y-14 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 shadow-lg add-to-cart z-10" data-product-id="' + p.id + '">' +
